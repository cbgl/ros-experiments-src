 ...
 */
alslogger_bag::alslogger_bag(void) : num_ground_truths_(0)
{
  loadParams();

  ground_truth_sub_ = nodehandle_.subscribe(ground_truth_topic_, 10,
    &alslogger_bag::groundTruthCallback, this);

  global_pose_sub_ = nodehandle_.subscribe(global_pose_topic_, 10,
    &alslogger_bag::globalPoseCallback, this);

  execution_time_sub_ = nodehandle_.subscribe(execution_time_topic_, 10,
    &alslogger_bag::executionTimeCallback, this);

  map_sub_ = nodehandle_.subscribe(map_topic_, 1,
    &alslogger_bag::mapCallback, this);

  initLogfiles();

  ROS_INFO("[alslogger_bag] Node initialised");
}
 ...
 * Destructor
 */
alslogger_bag::~alslogger_bag(void)
{
  ROS_INFO("[alslogger_bag] Node destroyed");
}
 ...
 * Logs execution time
 */
void alslogger_bag::executionTimeCallback(const std_msgs::Duration& msg)
{
  ...
  if (file.is_open())
  {
    ...
    ROS_ERROR("[alslogger_bag] Could not log execution time");
}
 ...
 */
void alslogger_bag::globalPoseCallback(
 ...
 */
void alslogger_bag::groundTruthCallback(
  const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  num_ground_truths_++;
 ...
 */
void alslogger_bag::initLogfiles(void)
{
  //----------------------------------------------------------------------------
  ...
  if (ground_truth_file.is_open())
  {
    ...
    ROS_ERROR("[alslogger_bag] Ground truth file not open");
  ...
  if (global_pose_file.is_open())
  {
    ...
    ROS_ERROR("[alslogger_bag] global pose file not open");
  ...
  if (execution_time_file.is_open())
  {
    ...
    ROS_ERROR("[alslogger_bag] execution-time file not open");
  ...
  if (map_file.is_open())
  {
    ...
    ROS_ERROR("[alslogger_bag] Map file not open");
}
 ...
 */
void alslogger_bag::loadParams(void)
{
  //----------------------------------------------------------------------------
  // Get topic names
  ...
  const std::string& filename)
{
  ...
  if (file.is_open())
  {
    ...
    ROS_ERROR("[alslogger_bag] Could not log pose");
}
 ...
 * Logs the map
 */
void alslogger_bag::mapCallback(
